...
sling_dir = os.path.join(templama_docker_dir, 'sling_dir')
sling_kb = sling_dir + "/data/e/kb/kb.sling"
sling_wiki = sling_dir + "/data/e/wiki/en/mapping.sling"
sling_kbfacts = templama_docker_dir + "/new_data1/wikidata/kb.cfacts"
...
flags.DEFINE_string("sling_kb_file",
                    sling_kb,
                    # None,
                    "SLING file containing wikidata KB.")
flags.DEFINE_string(
    "sling_wiki_mapping_file",
    sling_wiki,
    # None,
...
# flags.DEFINE_float("train_frac", 0.02,
#                    "Fraction of queries to hold out for training set.")
# flags.DEFINE_float("val_frac", 0.01,
...
def read_facts(facts_file, qid_mapping, min_year):
    ...
    with tf.io.gfile.GFile(facts_file) as f:
        for line in tqdm(f):
            ...
            # logging.info('fact len {}'.format(len(fact)))
            # Katerina addition / debugging
            if len(fact) <= 2:
                continue
...
def main(_):
    # Files to be created/saved
    qids_pt = os.path.join(FLAGS.out_dir, 'my_qids.pt')
    all_facts_pt = os.path.join(FLAGS.out_dir, 'my_all_facts.pt')

    if not os.path.isdir(FLAGS.out_dir):
        os.makedirs(FLAGS.out_dir)

    assert os.path.isfile(FLAGS.sling_kb_file)
    assert os.path.isfile(FLAGS.sling_wiki_mapping_file)
    assert os.path.isfile(FLAGS.facts_file)

    logging.info("Checking if qids_pickle file exists...")
    if os.path.isfile(qids_pt):
        ...
        logging.info("Not found! Loading sling mappings now...")
        ...
        torch.save([qid_names, qid_mapping, qid_numfacts],qids_pt)

    logging.info("len(qid_names)={}".format(len(qid_names)))
    logging.info("len(qid_mapping)={}".format(len(qid_mapping)))
    logging.info("len(qid_numfacts)={}".format(len(qid_numfacts)))

    # Load facts with qualifiers.
    logging.info("Checking if type all_facts_pickle file exists...")
    if os.path.isfile(all_facts_pt):
        logging.info("Found! Loading from {}...".format(all_facts_pt))
        all_facts = torch.load(all_facts_pt)
    else:
        logging.info("Not found! Loading facts with qualifiers from {} (min year) now...".format(FLAGS.min_year))
        all_facts = read_facts(FLAGS.facts_file, qid_mapping, FLAGS.min_year)
        torch.save(all_facts, os.path.join(FLAGS.out_dir, 'my_all_facts.pt'))

    logging.info("len(all_facts)={}".format(len(all_facts)))
    logging.info("Done! Check the {} directory for the created files.".format(FLAGS.out_dir))
